...
</head><body><header class="p-4"><div class="container d-flex justify-content-between flex-wrap gap-3"><a href="{{ route('home', ['locale' => $locale]) }}">@if($websiteSettings->logo_path)<img src="{{ asset($websiteSettings->logo_path) }}" alt="{{ $websiteSettings->title }}" style="max-width:180px;max-height:64px">@else{{ $websiteSettings->title }}@endif</a><a href="{{ route('news.index', ['locale' => $locale]) }}">{{ ['vi' => 'Tin tức', 'en' => 'News', 'zh' => '新闻', 'ko' => '뉴스'][$locale] }}</a></div></header><main>@yield('content')</main>{!! $websiteSettings->footer_code !!}
</body></html>
