...
@section('content')
<nav class="d-flex gap-3 mb-4">@foreach (\App\Models\NewsArticle::LANGUAGES as $code => $label)<a href="{{ route('news.index', ['locale' => $code]) }}" @if($code === $locale) aria-current="page" @endif>{{ $label }}</a>@endforeach</nav>
...
<div class="card-body"><time>{{ $article->published_at->format('d/m/Y') }}</time><h2 class="h4 mt-2"><a href="{{ route('news.show', ['locale' => $locale, 'news' => $article->id]) }}">{{ $article->title }}</a></h2><p>{{ $article->excerpt }}</p></div></article></div>
