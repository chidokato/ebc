...
@section('content')
<nav class="d-flex gap-3 mb-4">@foreach ($translations as $translation)<a href="{{ route('news.show', ['locale' => $translation->locale, 'news' => $translation->id]) }}" @if($translation->locale === $locale) aria-current="page" @endif>{{ \App\Models\NewsArticle::LANGUAGES[$translation->locale] }}</a>@endforeach</nav>
