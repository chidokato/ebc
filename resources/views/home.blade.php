...
<header class="elite-site-header fix-top">
	<nav class="elite-site-nav" aria-label="Điều hướng chính">
		<div class="container elite-nav-content">
			...
			<a class="elite-nav-logo" href="{{ route('home', ['locale' => $locale]) }}" aria-label="Elite Business Center"><img src="{{ asset($websiteSettings->white_logo_path ?: $websiteSettings->logo_path ?: 'frontend/img/logo-trang.png') }}" alt="{{ $websiteSettings->title }}"></a>
...
<section class="sec-ebc-benefits" id="about">
	<div class="container ebc-benefits-frame" @if($aboutSection?->images->isNotEmpty() || $aboutSection?->image_path) style="--about-background:url('{{ asset($aboutSection->images->first()?->path ?: $aboutSection->image_path) }}')" @endif>
		...
		<div class="ebc-benefit-slider">
			<div class="swiper">
				<div class="swiper-wrapper">
					@if($aboutSection?->children->count() >= 5)
